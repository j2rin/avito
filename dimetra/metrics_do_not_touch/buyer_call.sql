...
select
    ...
    sum(case when observation_name = 'answered_proxy_calls' and is_item_cpa = True then observation_value end) as answered_proxy_calls_cpa_items,
    sum(case when observation_name = 'answered_proxy_calls' and is_user_cpa = True then observation_value end) as answered_proxy_calls_cpa_users,
    sum(case when observation_name = 'answered_proxy_calls_fav' then observation_value end) as answered_proxy_calls_fav,
    sum(case when observation_name = 'answered_proxy_calls_fav' and is_item_cpa = True then observation_value end) as answered_proxy_calls_fav_cpa_items,
    sum(case when observation_name = 'answered_proxy_calls_fav' and is_user_cpa = True then observation_value end) as answered_proxy_calls_fav_cpa_users,
    sum(case when observation_name = 'answered_proxy_calls_map' then observation_value end) as answered_proxy_calls_map,
    sum(case when observation_name = 'answered_proxy_calls_map' and is_item_cpa = True then observation_value end) as answered_proxy_calls_map_cpa_items,
    sum(case when observation_name = 'answered_proxy_calls_map' and is_user_cpa = True then observation_value end) as answered_proxy_calls_map_cpa_users,
    sum(case when observation_name = 'answered_proxy_calls_rec' then observation_value end) as answered_proxy_calls_rec,
    sum(case when observation_name = 'answered_proxy_calls_rec' and is_item_cpa = True then observation_value end) as answered_proxy_calls_rec_cpa_items,
    sum(case when observation_name = 'answered_proxy_calls_rec' and is_user_cpa = True then observation_value end) as answered_proxy_calls_rec_cpa_users,
    sum(case when observation_name = 'answered_proxy_calls_serp' then observation_value end) as answered_proxy_calls_serp,
    sum(case when observation_name = 'answered_proxy_calls_serp' and is_item_cpa = True then observation_value end) as answered_proxy_calls_serp_cpa_items,
    sum(case when observation_name = 'answered_proxy_calls_serp' and is_user_cpa = True then observation_value end) as answered_proxy_calls_serp_cpa_users,
    ...
    sum(case when is_item_cpa = True and (observation_name in ('answered_proxy_calls', 'call_phone_screen_views_client', 'missed_proxy_calls')) then observation_value end) as calls_cpa_items,
    sum(case when is_user_cpa = True and (observation_name in ('answered_proxy_calls', 'call_phone_screen_views_client', 'missed_proxy_calls')) then observation_value end) as calls_cpa_users,
    ...
    sum(case when is_item_cpa = True and (observation_name in ('answered_proxy_calls_fav', 'missed_proxy_calls_fav')) then observation_value end) as calls_fav_cpa_items,
    sum(case when is_user_cpa = True and (observation_name in ('answered_proxy_calls_fav', 'missed_proxy_calls_fav')) then observation_value end) as calls_fav_cpa_users,
    ...
    sum(case when is_item_cpa = True and (observation_name in ('answered_proxy_calls_map', 'missed_proxy_calls_map')) then observation_value end) as calls_map_cpa_items,
    sum(case when is_user_cpa = True and (observation_name in ('answered_proxy_calls_map', 'missed_proxy_calls_map')) then observation_value end) as calls_map_cpa_users,
    ...
    sum(case when is_item_cpa = True and (observation_name in ('answered_proxy_calls_rec', 'missed_proxy_calls_rec')) then observation_value end) as calls_rec_cpa_items,
    sum(case when is_user_cpa = True and (observation_name in ('answered_proxy_calls_rec', 'missed_proxy_calls_rec')) then observation_value end) as calls_rec_cpa_users,
    ...
    sum(case when is_item_cpa = True and (observation_name in ('answered_proxy_calls_serp', 'missed_proxy_calls_serp')) then observation_value end) as calls_serp_cpa_items,
    sum(case when is_user_cpa = True and (observation_name in ('answered_proxy_calls_serp', 'missed_proxy_calls_serp')) then observation_value end) as calls_serp_cpa_users,
    sum(case when observation_name = 'missed_proxy_calls' then observation_value end) as missed_proxy_calls,
    sum(case when observation_name = 'missed_proxy_calls' and is_item_cpa = True then observation_value end) as missed_proxy_calls_cpa_items,
    sum(case when observation_name = 'missed_proxy_calls' and is_user_cpa = True then observation_value end) as missed_proxy_calls_cpa_users,
    sum(case when observation_name = 'missed_proxy_calls_fav' then observation_value end) as missed_proxy_calls_fav,
    sum(case when observation_name = 'missed_proxy_calls_fav' and is_item_cpa = True then observation_value end) as missed_proxy_calls_fav_cpa_items,
    sum(case when observation_name = 'missed_proxy_calls_fav' and is_user_cpa = True then observation_value end) as missed_proxy_calls_fav_cpa_users,
    sum(case when observation_name = 'missed_proxy_calls_map' then observation_value end) as missed_proxy_calls_map,
    sum(case when observation_name = 'missed_proxy_calls_map' and is_item_cpa = True then observation_value end) as missed_proxy_calls_map_cpa_items,
    sum(case when observation_name = 'missed_proxy_calls_map' and is_user_cpa = True then observation_value end) as missed_proxy_calls_map_cpa_users,
    sum(case when observation_name = 'missed_proxy_calls_rec' then observation_value end) as missed_proxy_calls_rec,
    sum(case when observation_name = 'missed_proxy_calls_rec' and is_item_cpa = True then observation_value end) as missed_proxy_calls_rec_cpa_items,
    sum(case when observation_name = 'missed_proxy_calls_rec' and is_user_cpa = True then observation_value end) as missed_proxy_calls_rec_cpa_users,
    sum(case when observation_name = 'missed_proxy_calls_serp' then observation_value end) as missed_proxy_calls_serp,
    sum(case when observation_name = 'missed_proxy_calls_serp' and is_item_cpa = True then observation_value end) as missed_proxy_calls_serp_cpa_items,
    sum(case when observation_name = 'missed_proxy_calls_serp' and is_user_cpa = True then observation_value end) as missed_proxy_calls_serp_cpa_users
from buyer_call t
;

create metrics buyer_call_item as
select
    sum(case when calls_cpa_items > 0 then 1 end) as items_cpa_phone_calls,
    sum(case when calls_fav_cpa_items > 0 then 1 end) as items_cpa_phone_calls_fav,
    sum(case when calls_map_cpa_items > 0 then 1 end) as items_cpa_phone_calls_map,
    sum(case when calls_serp_cpa_items > 0 then 1 end) as items_cpa_phone_calls_serp
from (
    select
        cookie_id, item_id,
        sum(case when is_item_cpa = True and (observation_name in ('answered_proxy_calls', 'call_phone_screen_views_client', 'missed_proxy_calls')) then observation_value end) as calls_cpa_items,
...
select
    ...
    sum(case when calls_cpa_items > 0 then 1 end) as users_phone_calls_client_cpa_items,
    sum(case when calls_cpa_users > 0 then 1 end) as users_phone_calls_client_cpa_users,
    sum(case when calls_fav > 0 then 1 end) as users_phone_calls_client_fav,
    sum(case when calls_fav_cpa_items > 0 then 1 end) as users_phone_calls_client_fav_cpa_items,
    sum(case when calls_fav_cpa_users > 0 then 1 end) as users_phone_calls_client_fav_cpa_users,
    sum(case when calls_map > 0 then 1 end) as users_phone_calls_client_map,
    sum(case when calls_map_cpa_items > 0 then 1 end) as users_phone_calls_client_map_cpa_items,
    sum(case when calls_map_cpa_users > 0 then 1 end) as users_phone_calls_client_map_cpa_users,
    sum(case when calls_serp > 0 then 1 end) as users_phone_calls_client_serp,
    sum(case when calls_serp_cpa_items > 0 then 1 end) as users_phone_calls_client_serp_cpa_items,
    sum(case when calls_serp_cpa_users > 0 then 1 end) as users_phone_calls_client_serp_cpa_users
from (
    select
