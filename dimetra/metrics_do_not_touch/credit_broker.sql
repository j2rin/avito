...
from (
    select
        ...
        sum(case when event_name_slug = 'broker_credit_approved' and x_from_page in ('moneyman', 'sravni_cash', 'tinkoff_cash') then 1 end) as credit_cash_full_approvals,
        sum(case when eventtype_ext = 4498 and x_from_page in ('moneyman', 'sravni_cash', 'tinkoff_cash') then event_count end) as credit_cash_full_clicks,
        sum(case when eventtype_ext = 4590 and x_from_page in ('moneyman', 'sravni_cash', 'tinkoff_cash') then event_count end) as credit_cash_full_fillings,
        sum(case when is_issued = 1 and x_from_page in ('moneyman', 'sravni_cash', 'tinkoff_cash') then 1 end) as credit_cash_full_issues,
        sum(case when eventtype_ext = 5283 and x_from_page in ('moneyman', 'sravni_cash', 'tinkoff_cash') then event_count end) as credit_cash_full_loads,
        sum(case when eventtype_ext = 4801 and x_from_page in ('moneyman', 'sravni_cash', 'tinkoff_cash') then event_count end) as credit_cash_full_logo_clicks,
        sum(case when eventtype_ext = 4496 and x_from_page in ('moneyman', 'sravni_cash', 'tinkoff_cash') then event_count end) as credit_cash_full_renders,
        sum(case when eventtype_ext = 4502 and x_from_page in ('moneyman', 'sravni_cash', 'tinkoff_cash') then event_count end) as credit_cash_full_requests
...
select
    ...
    sum(case when event_name_slug = 'broker_credit_approved' and x_from_page in ('moneyman', 'sravni_cash', 'tinkoff_cash') then 1 end) as credit_cash_full_approvals,
    sum(case when eventtype_ext = 4498 and x_from_page in ('moneyman', 'sravni_cash', 'tinkoff_cash') then event_count end) as credit_cash_full_clicks,
    sum(case when is_early_closed = 1 and x_from_page in ('moneyman', 'sravni_cash', 'tinkoff_cash') then 1 end) as credit_cash_full_early_close,
    sum(case when eventtype_ext = 4590 and x_from_page in ('moneyman', 'sravni_cash', 'tinkoff_cash') then event_count end) as credit_cash_full_fillings,
    sum(case when is_issued = 1 and x_from_page in ('moneyman', 'sravni_cash', 'tinkoff_cash') then 1 end) as credit_cash_full_issues,
    sum(case when eventtype_ext = 5283 and x_from_page in ('moneyman', 'sravni_cash', 'tinkoff_cash') then event_count end) as credit_cash_full_loads,
    sum(case when eventtype_ext = 4801 and x_from_page in ('moneyman', 'sravni_cash', 'tinkoff_cash') then event_count end) as credit_cash_full_logo_clicks,
    sum(case when eventtype_ext = 4496 and x_from_page in ('moneyman', 'sravni_cash', 'tinkoff_cash') then event_count end) as credit_cash_full_renders,
    sum(case when eventtype_ext = 4502 and x_from_page in ('moneyman', 'sravni_cash', 'tinkoff_cash') then event_count end) as credit_cash_full_requests,
    sum(case when is_issued = 1 and is_early_closed is null and x_from_page in ('moneyman', 'sravni_cash', 'tinkoff_cash') then revenue end) as credit_cash_revenue
from credit_broker t
;
