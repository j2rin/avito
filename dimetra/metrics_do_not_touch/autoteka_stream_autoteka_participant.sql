...
select
    ...
    sum(case when autoteka_cnt_reports_search_item_ap > 0 then 1 end) as autoteka_unique_report_buyers_search_item_ap,
...
from (
    select
        ...
        sum(case when funnel_stage_id = 4 and searchtype = 1 then reports_count end) as autoteka_cnt_reports_search_item_ap,
...
select
    ...
    sum(case when funnel_stage_id = 1 and searchtype = 1 then 1 end) as autoteka_cnt_previews_search_item_ap,
    ...
    sum(case when funnel_stage_id = 4 and searchtype = 1 then amount end) as autoteka_revenue_search_item_ap
from autoteka_stream_autoteka_participant t
;
...
select
    ...
    sum(case when autoteka_cnt_previews_search_item_ap > 0 then 1 end) as autoteka_unique_preview_visitors_search_item_ap,
