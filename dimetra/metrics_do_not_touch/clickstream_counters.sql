...
select
    ...
    sum(case when eid = 4920 and from_page = 'shortcuts_widget' then event_count end) as show_shortcuts_widget,
    ...
    sum(case when eid = 6071 then event_count end) as total_osp_back_to_item,
    sum(case when eid = 5783 then event_count end) as total_osp_close,
    sum(case when eid = 5762 then event_count end) as total_osp_open,
    sum(case when eid = 5785 then event_count end) as total_osp_order_details,
    sum(case when eid = 5784 then event_count end) as total_osp_write_seller,
...
select
    ...
    sum(case when cnt_order_page_views_total > 0 then 1 end) as user_order_page_views,
    ...
    sum(case when show_shortcuts_widget > 0 then 1 end) as user_show_shortcuts_widget,
...
create metrics clickstream_counters_cookie_order as
select
    sum(case when cnt_order_page_views_from_messenger > 0 then 1 end) as order_page_views_from_messenger,
    sum(case when cnt_order_page_views_from_my_orders > 0 then 1 end) as order_page_views_from_my_orders,
    sum(case when cnt_order_page_views_from_success_page > 0 then 1 end) as order_page_views_from_success_page,
    sum(case when cnt_order_page_views_total > 0 then 1 end) as order_page_views_total
from (
    select
        cookie_id, orderid,
        sum(case when eid = 4760 and source_click_page = 'messenger' then 1 end) as cnt_order_page_views_from_messenger,
        ...
        sum(case when eid = 4760 then 1 end) as cnt_order_page_views_total
    from clickstream_counters t
    group by cookie_id, orderid
) _
;

create metrics clickstream_counters_orderid_string as
select
    sum(case when total_osp_back_to_item > 0 then 1 end) as osp_back_to_item,
    sum(case when total_osp_close > 0 then 1 end) as osp_close,
    sum(case when total_osp_open > 0 then 1 end) as osp_open,
    sum(case when total_osp_order_details > 0 then 1 end) as osp_order_details,
    sum(case when total_osp_write_seller > 0 then 1 end) as osp_write_seller
from (
    select
        cookie_id, orderid_string,
        sum(case when eid = 6071 then event_count end) as total_osp_back_to_item,
        sum(case when eid = 5783 then event_count end) as total_osp_close,
        sum(case when eid = 5762 then event_count end) as total_osp_open,
        sum(case when eid = 5785 then event_count end) as total_osp_order_details,
        sum(case when eid = 5784 then event_count end) as total_osp_write_seller
    from clickstream_counters t
    group by cookie_id, orderid_string
) _
;
