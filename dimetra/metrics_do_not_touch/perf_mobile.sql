...
select
    ...
    sum(case when stage_name = 'Draw' and error_dialog_type != 'ordinary error' and is_core_content = True and (response not in ('no-data-placeholder', 'ok') or response = 'no-data-placeholder' and was_error_in_screen = True) then ifnull(events, 0) + ifnull(exceptions, 0) end) as dialog_core_exceptions,
    sum(case when stage_name = 'Draw' and error_dialog_type != 'ordinary error' and (response not in ('no-data-placeholder', 'ok') or response = 'no-data-placeholder' and was_error_in_screen = True) then ifnull(events, 0) + ifnull(exceptions, 0) end) as dialog_exceptions,
    sum(case when stage_name = 'Draw' and error_dialog_type != 'ordinary error' and response = 'failed-to-load-error' then ifnull(events, 0) + ifnull(exceptions, 0) end) as dialog_exceptions_failed_to_load,
    ...
    sum(case when stage_name = 'Draw' and error_dialog_type != 'ordinary error' and response = 'no-internet-error' then ifnull(events, 0) + ifnull(exceptions, 0) end) as dialog_exceptions_no_internet,
    sum(case when stage_name = 'Draw' and error_dialog_type != 'ordinary error' and response = 'somehow-else-displayed-error' then ifnull(events, 0) + ifnull(exceptions, 0) end) as dialog_exceptions_somehow_else_displayed,
    sum(case when stage_name = 'Draw' and error_dialog_type != 'ordinary error' and response = 'unknown-error' then ifnull(events, 0) + ifnull(exceptions, 0) end) as dialog_exceptions_unknown,
    ...
    sum(case when stage_name = 'Draw' and error_dialog_type = 'toast' and is_core_content = True and (response not in ('no-data-placeholder', 'ok') or response = 'no-data-placeholder' and was_error_in_screen = True) then ifnull(events, 0) + ifnull(exceptions, 0) end) as toast_core_exceptions,
    sum(case when stage_name = 'Draw' and error_dialog_type = 'toast' and (response not in ('no-data-placeholder', 'ok') or response = 'no-data-placeholder' and was_error_in_screen = True) then ifnull(events, 0) + ifnull(exceptions, 0) end) as toast_exceptions,
...
select
    ...
    sum(case when cnt_draw_events_exceptions > 0 then 1 end) as user_draw_all_events,
