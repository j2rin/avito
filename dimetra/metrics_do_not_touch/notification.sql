...
select
    ...
    sum(browser_pushes_marketing_to_receiver) as browser_pushes_marketing_to_receiver,
    sum(email_action) as email_action,
    sum(email_marketing_to_receiver) as email_marketing_to_receiver,
    ...
    sum(notification_center_marketing_to_receiver) as nc_marketing_to_receiver,
    ...
    sum(notification_crm_action) as notification_crm_action,
    sum(notification_crm_no_data) as notification_crm_no_data,
    sum(notification_crm_open) as notification_crm_open,
    sum(notification_crm_prepared) as notification_crm_prepared,
    sum(notification_crm_to_provider) as notification_crm_to_provider,
    sum(notification_crm_to_receiver) as notification_crm_to_receiver,
    sum(notification_marketing_action) as notification_marketing_action,
    sum(notification_marketing_no_data) as notification_marketing_no_data,
    sum(notification_marketing_open) as notification_marketing_open,
    sum(notification_marketing_prepared) as notification_marketing_prepared,
    sum(notification_marketing_to_provider) as notification_marketing_to_provider,
    sum(notification_marketing_to_receiver) as notification_marketing_to_receiver,
    sum(notification_rec_action) as notification_rec_action,
    sum(notification_rec_no_data) as notification_rec_no_data,
    sum(notification_rec_open) as notification_rec_open,
    sum(notification_rec_prepared) as notification_rec_prepared,
    sum(notification_rec_to_provider) as notification_rec_to_provider,
    sum(notification_rec_to_receiver) as notification_rec_to_receiver,
    sum(app_pushes_crm_to_receiver) as push_crm_to_receiver,
    sum(app_pushes_marketing_to_receiver) as push_marketing_to_receiver,
    ...
    sum(app_pushes_rec_to_receiver) as push_rec_to_receiver,
    ...
    sum(sms_marketing_to_receiver) as sms_marketing_to_receiver,
...
select
    ...
    sum(case when browser_pushes_marketing_to_receiver > 0 then 1 end) as user_browser_pushes_marketing_to_receiver,
    sum(case when email_action > 0 then 1 end) as user_email_action,
    sum(case when email_marketing_to_receiver > 0 then 1 end) as user_email_marketing_to_receiver,
    ...
    sum(case when nc_marketing_to_receiver > 0 then 1 end) as user_nc_marketing_to_receiver,
    ...
    sum(case when notification_crm_action > 0 then 1 end) as user_notification_crm_action,
    sum(case when notification_crm_no_data > 0 then 1 end) as user_notification_crm_no_data,
    sum(case when notification_crm_open > 0 then 1 end) as user_notification_crm_open,
    sum(case when notification_crm_prepared > 0 then 1 end) as user_notification_crm_prepared,
    sum(case when notification_crm_to_provider > 0 then 1 end) as user_notification_crm_to_provider,
    sum(case when notification_crm_to_receiver > 0 then 1 end) as user_notification_crm_to_receiver,
    sum(case when notification_marketing_action > 0 then 1 end) as user_notification_marketing_action,
    sum(case when notification_marketing_no_data > 0 then 1 end) as user_notification_marketing_no_data,
    sum(case when notification_marketing_open > 0 then 1 end) as user_notification_marketing_open,
    sum(case when notification_marketing_prepared > 0 then 1 end) as user_notification_marketing_prepared,
    sum(case when notification_marketing_to_provider > 0 then 1 end) as user_notification_marketing_to_provider,
    sum(case when notification_marketing_to_receiver > 0 then 1 end) as user_notification_marketing_to_receiver,
    sum(case when notification_rec_action > 0 then 1 end) as user_notification_rec_action,
    sum(case when notification_rec_no_data > 0 then 1 end) as user_notification_rec_no_data,
    sum(case when notification_rec_open > 0 then 1 end) as user_notification_rec_open,
    sum(case when notification_rec_prepared > 0 then 1 end) as user_notification_rec_prepared,
    sum(case when notification_rec_to_provider > 0 then 1 end) as user_notification_rec_to_provider,
    sum(case when notification_rec_to_receiver > 0 then 1 end) as user_notification_rec_to_receiver,
    sum(case when push_crm_to_receiver > 0 then 1 end) as user_push_crm_to_receiver,
    sum(case when push_marketing_to_receiver > 0 then 1 end) as user_push_marketing_to_receiver,
    ...
    sum(case when push_rec_to_receiver > 0 then 1 end) as user_push_rec_to_receiver,
    ...
    sum(case when sms_marketing_to_receiver > 0 then 1 end) as user_sms_marketing_to_receiver,
...
from (
    select
        ...
        sum(sms_delivered_to_receiver) as sms_to_receiver
    from notification t
    group by user_id, participant_id
) _
;
