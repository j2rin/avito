...
select
    ...
    sum(case when is_accepted = True then c2c_seller_discount end) as seller_delivery_c2c_seller_discount_accepted,
    sum(case when (is_accepted = True or is_returned = True) then c2c_seller_discount end) as seller_delivery_c2c_seller_discount_accepted_and_returned,
    sum(case when is_created = True then c2c_seller_discount end) as seller_delivery_c2c_seller_discount_created,
    sum(case when is_paid = True then c2c_seller_discount end) as seller_delivery_c2c_seller_discount_paid,
    sum(case when is_performed = True then c2c_seller_discount end) as seller_delivery_c2c_seller_discount_performed,
    sum(case when is_returned = True then c2c_seller_discount end) as seller_delivery_c2c_seller_discount_returned,
    ...
    sum(case when is_paid = True and is_c2c_seller_discount = True then item_price end) as seller_delivery_gmv_c2c_seller_discount_paid,
    ...
    sum(case when is_accepted = True and is_c2c_seller_discount = True then 1 end) as seller_delivery_items_c2c_seller_discount_accepted,
    sum(case when is_c2c_seller_discount = True and (is_accepted = True or is_returned = True) then 1 end) as seller_delivery_items_c2c_seller_discount_accepted_and_returned,
    sum(case when is_created = True and is_c2c_seller_discount = True then 1 end) as seller_delivery_items_c2c_seller_discount_created,
    sum(case when is_paid = True and is_c2c_seller_discount = True then 1 end) as seller_delivery_items_c2c_seller_discount_paid,
    sum(case when is_performed = True and is_c2c_seller_discount = True then 1 end) as seller_delivery_items_c2c_seller_discount_performed,
    sum(case when is_returned = True and is_c2c_seller_discount = True then 1 end) as seller_delivery_items_c2c_seller_discount_returned,
    ...
    sum(case when is_accepted = True and is_delivery_discount = True then 1 end) as seller_delivery_items_delivery_discount_accepted,
    sum(case when is_delivery_discount = True and (is_accepted = True or is_returned = True) then 1 end) as seller_delivery_items_delivery_discount_accepted_and_returned,
    sum(case when is_created = True and is_delivery_discount = True then 1 end) as seller_delivery_items_delivery_discount_created,
    sum(case when is_paid = True and is_delivery_discount = True then 1 end) as seller_delivery_items_delivery_discount_paid,
    sum(case when is_performed = True and is_delivery_discount = True then 1 end) as seller_delivery_items_delivery_discount_performed,
    sum(case when is_returned = True and is_delivery_discount = True then 1 end) as seller_delivery_items_delivery_discount_returned,
    sum(case when is_accepted = True and total_subsidies > 0 then 1 end) as seller_delivery_items_delivery_subsidies_accepted,
    sum(case when total_subsidies > 0 and (is_accepted = True or is_returned = True) then 1 end) as seller_delivery_items_delivery_subsidies_accepted_and_returned,
    sum(case when is_created = True and total_subsidies > 0 then 1 end) as seller_delivery_items_delivery_subsidies_created,
    sum(case when is_paid = True and total_subsidies > 0 then 1 end) as seller_delivery_items_delivery_subsidies_paid,
    sum(case when is_performed = True and total_subsidies > 0 then 1 end) as seller_delivery_items_delivery_subsidies_performed,
    sum(case when is_returned = True and total_subsidies > 0 then 1 end) as seller_delivery_items_delivery_subsidies_returned,
    ...
    sum(case when is_accepted = True then total_subsidies end) as seller_delivery_subsidies_accepted,
    sum(case when (is_accepted = True or is_returned = True) then total_subsidies end) as seller_delivery_subsidies_accepted_and_returned,
    sum(case when is_created = True then total_subsidies end) as seller_delivery_subsidies_created,
    sum(case when is_paid = True then total_subsidies end) as seller_delivery_subsidies_paid,
    sum(case when is_performed = True then total_subsidies end) as seller_delivery_subsidies_performed,
    sum(case when is_returned = True then total_subsidies end) as seller_delivery_subsidies_returned
...
select
    ...
    sum(case when seller_delivery_items_c2c_seller_discount_paid > 0 then 1 end) as seller_delivery_orders_c2c_seller_discount_paid,
    ...
    sum(case when seller_delivery_items_delivery_discount_paid > 0 then 1 end) as seller_delivery_orders_delivery_discount_paid,
