...
select
    sum(case when close_status_id = 7 and start_date >= :ab_start_date then 1 end) as cnt_items_closed_by_user,
    sum(case when activation_type != 'Wait' and is_dead = False and is_item_start = True and start_date >= :ab_start_date then 1 end) as cnt_items_started_net,
    ...
    sum(case when start_date >= :ab_start_date then edits_description end) as item_edits_description_start_date,
    sum(edits_photo) as item_edits_photo_add,
    sum(case when start_date >= :ab_start_date then edits_photo end) as item_edits_photo_add_start_date,
    sum(edits_price) as item_edits_price,
    sum(case when start_date >= :ab_start_date then edits_price end) as item_edits_price_start_date,
    sum(case when start_date >= :ab_start_date then edits end) as item_edits_start_date,
    sum(edits_title) as item_edits_title,
    sum(case when start_date >= :ab_start_date then edits_title end) as item_edits_title_start_date,
    ...
    sum(case when moder_flags & 7 > 0 and start_date >= :ab_start_date then 1 end) as items_allowed_start_date,
    ...
    sum(case when moder_flags & 32 > 0 and start_date >= :ab_start_date then 1 end) as items_blocked_duplicate_start_date,
    ...
    sum(case when moder_flags & 72 > 0 and start_date >= :ab_start_date then 1 end) as items_blocked_rejected_start_date,
    sum(case when moder_flags & 8 > 0 and start_date >= :ab_start_date then 1 end) as items_blocked_start_date,
    sum(case when moder_flags & 16 > 0 then 1 end) as items_blocked_user_blocked,
    sum(case when moder_flags & 16 > 0 and start_date >= :ab_start_date then 1 end) as items_blocked_user_blocked_start_date,
    sum(case when moder_flags & 131072 > 0 then 1 end) as items_category_changed_by_moderator,
    sum(case when moder_flags & 131072 > 0 and start_date >= :ab_start_date then 1 end) as items_category_changed_by_moderator_start_date,
    ...
    sum(case when moder_flags & 2048 > 0 and start_date >= :ab_start_date then 1 end) as items_rejected_contacts_in_description_start_date,
    sum(case when moder_flags & 16384 > 0 then 1 end) as items_rejected_contacts_on_photo,
    sum(case when moder_flags & 16384 > 0 and start_date >= :ab_start_date then 1 end) as items_rejected_contacts_on_photo_start_date,
    ...
    sum(case when moder_flags & 65536 > 0 and start_date >= :ab_start_date then 1 end) as items_rejected_mileage_start_date,
    sum(case when moder_flags & 128 > 0 then 1 end) as items_rejected_phone_in_other_profile,
    sum(case when moder_flags & 128 > 0 and start_date >= :ab_start_date then 1 end) as items_rejected_phone_in_other_profile_start_date,
    sum(case when moder_flags & 512 > 0 then 1 end) as items_rejected_pricelist,
    sum(case when moder_flags & 512 > 0 and start_date >= :ab_start_date then 1 end) as items_rejected_pricelist_start_date,
    sum(case when moder_flags & 64 > 0 and start_date >= :ab_start_date then 1 end) as items_rejected_start_date,
    sum(case when moder_flags & 256 > 0 then 1 end) as items_rejected_wrong_cat,
    sum(case when moder_flags & 256 > 0 and start_date >= :ab_start_date then 1 end) as items_rejected_wrong_cat_start_date,
    sum(case when moder_flags & 1024 > 0 then 1 end) as items_rejected_wrong_item_type,
    sum(case when moder_flags & 1024 > 0 and start_date >= :ab_start_date then 1 end) as items_rejected_wrong_item_type_start_date,
    sum(case when moder_flags & 8192 > 0 then 1 end) as items_rejected_wrong_price,
    sum(case when moder_flags & 8192 > 0 and start_date >= :ab_start_date then 1 end) as items_rejected_wrong_price_start_date,
    sum(case when moder_flags & 4096 > 0 then 1 end) as items_rejected_wrong_vin,
    sum(case when moder_flags & 4096 > 0 and start_date >= :ab_start_date then 1 end) as items_rejected_wrong_vin_start_date,
    sum(case when moder_flags & 32768 > 0 then 1 end) as items_rejected_year_of_issue,
    sum(case when moder_flags & 32768 > 0 and start_date >= :ab_start_date then 1 end) as items_rejected_year_of_issue_start_date,
    sum(case when close_reason_id = 500012 then 1 end) as items_sold_another_reason,
    sum(case when close_reason_id = 500012 and start_date >= :ab_start_date then 1 end) as items_sold_another_reason_start_date,
    sum(case when close_reason_id = 500011 then 1 end) as items_sold_notonavito,
    sum(case when close_reason_id = 500011 and start_date >= :ab_start_date then 1 end) as items_sold_notonavito_start_date,
    sum(case when close_reason_id = 500004 then 1 end) as items_sold_onavito,
    sum(case when close_reason_id = 500004 and start_date >= :ab_start_date then 1 end) as items_sold_onavito_start_date,
    ...
    sum(case when start_date >= :ab_start_date then lf_amount_net end) as lf_amount_net_start_date
...
select
    ...
    sum(case when item_edits_description_start_date > 0 then 1 end) as items_edited_description_start_date,
    sum(case when item_edits_photo_add > 0 then 1 end) as items_edited_photo_add,
    sum(case when item_edits_photo_add_start_date > 0 then 1 end) as items_edited_photo_add_start_date,
    sum(case when item_edits_price > 0 then 1 end) as items_edited_price,
    sum(case when item_edits_price_start_date > 0 then 1 end) as items_edited_price_start_date,
    sum(case when item_edits_start_date > 0 then 1 end) as items_edited_start_date,
    sum(case when item_edits_title > 0 then 1 end) as items_edited_title,
    sum(case when item_edits_title_start_date > 0 then 1 end) as items_edited_title_start_date,
...
select
    ...
    sum(case when items_allowed_start_date > 0 then 1 end) as users_item_allowed_start_date,
    ...
    sum(case when items_blocked_duplicate_start_date > 0 then 1 end) as users_item_blocked_duplicate_start_date,
    sum(case when items_blocked_rejected > 0 then 1 end) as users_item_blocked_rejected,
    sum(case when items_blocked_rejected_start_date > 0 then 1 end) as users_item_blocked_rejected_start_date,
    sum(case when items_blocked_start_date > 0 then 1 end) as users_item_blocked_start_date,
    sum(case when items_blocked_user_blocked > 0 then 1 end) as users_item_blocked_user_blocked,
    sum(case when items_blocked_user_blocked_start_date > 0 then 1 end) as users_item_blocked_user_blocked_start_date,
    sum(case when items_category_changed_by_moderator > 0 then 1 end) as users_item_category_changed_by_moderator,
    sum(case when items_category_changed_by_moderator_start_date > 0 then 1 end) as users_item_category_changed_by_moderator_start_date,
    ...
    sum(case when item_edits_description_start_date > 0 then 1 end) as users_item_edit_description_start_date,
    sum(case when item_edits_photo_add > 0 then 1 end) as users_item_edit_photo_add,
    sum(case when item_edits_photo_add_start_date > 0 then 1 end) as users_item_edit_photo_add_start_date,
    sum(case when item_edits_price > 0 then 1 end) as users_item_edit_price,
    sum(case when item_edits_price_start_date > 0 then 1 end) as users_item_edit_price_start_date,
    ...
    sum(case when item_edits_start_date > 0 then 1 end) as users_item_edit_start_date,
    sum(case when item_edits_title > 0 then 1 end) as users_item_edit_title,
    sum(case when item_edits_title_start_date > 0 then 1 end) as users_item_edit_title_start_date,
    ...
    sum(case when items_rejected_contacts_in_description_start_date > 0 then 1 end) as users_item_rejected_contacts_in_description_start_date,
    sum(case when items_rejected_contacts_on_photo > 0 then 1 end) as users_item_rejected_contacts_on_photo,
    sum(case when items_rejected_contacts_on_photo_start_date > 0 then 1 end) as users_item_rejected_contacts_on_photo_start_date,
    sum(case when items_rejected_mileage > 0 then 1 end) as users_item_rejected_mileage,
    sum(case when items_rejected_mileage_start_date > 0 then 1 end) as users_item_rejected_mileage_start_date,
    sum(case when items_rejected_phone_in_other_profile > 0 then 1 end) as users_item_rejected_phone_in_other_profile,
    sum(case when items_rejected_phone_in_other_profile_start_date > 0 then 1 end) as users_item_rejected_phone_in_other_profile_start_date,
    sum(case when items_rejected_pricelist > 0 then 1 end) as users_item_rejected_pricelist,
    sum(case when items_rejected_pricelist_start_date > 0 then 1 end) as users_item_rejected_pricelist_start_date,
    sum(case when items_rejected_start_date > 0 then 1 end) as users_item_rejected_start_date,
    sum(case when items_rejected_wrong_cat > 0 then 1 end) as users_item_rejected_wrong_cat,
    sum(case when items_rejected_wrong_cat_start_date > 0 then 1 end) as users_item_rejected_wrong_cat_start_date,
    sum(case when items_rejected_wrong_item_type > 0 then 1 end) as users_item_rejected_wrong_item_type,
    sum(case when items_rejected_wrong_item_type_start_date > 0 then 1 end) as users_item_rejected_wrong_item_type_start_date,
    sum(case when items_rejected_wrong_price > 0 then 1 end) as users_item_rejected_wrong_price,
    sum(case when items_rejected_wrong_price_start_date > 0 then 1 end) as users_item_rejected_wrong_price_start_date,
    sum(case when items_rejected_wrong_vin > 0 then 1 end) as users_item_rejected_wrong_vin,
    sum(case when items_rejected_wrong_vin_start_date > 0 then 1 end) as users_item_rejected_wrong_vin_start_date,
    sum(case when items_rejected_year_of_issue > 0 then 1 end) as users_item_rejected_year_of_issue,
    sum(case when items_rejected_year_of_issue_start_date > 0 then 1 end) as users_item_rejected_year_of_issue_start_date,
    sum(case when items_sold_another_reason > 0 then 1 end) as users_item_sold_another_reason,
    sum(case when items_sold_another_reason_start_date > 0 then 1 end) as users_item_sold_another_reason_start_date,
    ...
    sum(case when lf_amount_net_start_date > 0 then 1 end) as users_lf_start_date
from (
    select
        user_id, user,
        sum(edits_started) as item_edit_starts,
