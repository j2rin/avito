...
select
    ...
    sum(case when is_active = True and is_delivery_available = True and start_date >= :ab_start_date then 1 end) as active_items_deliverable_start_date,
    sum(case when is_active = True and is_delivery_active = True and start_date >= :ab_start_date then 1 end) as active_items_delivery_on_start_date,
    ...
    sum(case when start_date >= :ab_start_date then contacts_msg end) as seller_contacts_messenger_start_date,
    sum(case when is_delivery_available = True then contacts end) as seller_contacts_on_deliverable_items,
    sum(case when is_delivery_available = True and start_date >= :ab_start_date then contacts end) as seller_contacts_on_deliverable_items_start_date,
    sum(case when is_delivery_active = True then contacts end) as seller_contacts_on_delivery_items,
    sum(case when is_delivery_active = True and start_date >= :ab_start_date then contacts end) as seller_contacts_on_delivery_items_start_date,
    sum(contacts_ph) as seller_contacts_phone_views,
    sum(case when start_date >= :ab_start_date then contacts_ph end) as seller_contacts_phone_views_start_date,
    sum(case when start_date >= :ab_start_date then contacts end) as seller_contacts_start_date,
    ...
    sum(case when is_delivery_available = True and start_date >= :ab_start_date then item_views end) as seller_deliverable_iv_start_date,
    sum(case when is_delivery_available = True then item_views end) as seller_delivery_available_iv,
    sum(delivery_clicks) as seller_delivery_clicks,
    sum(case when start_date >= :ab_start_date then delivery_clicks end) as seller_delivery_clicks_start_date,
    sum(delivery_contacts) as seller_delivery_contacts,
    sum(case when start_date >= :ab_start_date then delivery_contacts end) as seller_delivery_contacts_start_date,
    sum(case when is_delivery_active = True then item_views end) as seller_delivery_iv,
    sum(case when is_delivery_active = True and start_date >= :ab_start_date then item_views end) as seller_delivery_iv_start_date,
    sum(favs_added) as seller_favorites_added,
    sum(case when start_date >= :ab_start_date then favs_added end) as seller_favorites_added_start_date,
    sum(favs_removed) as seller_favorites_removed,
    sum(case when start_date >= :ab_start_date then favs_removed end) as seller_favorites_removed_start_date,
    sum(item_views) as seller_item_views,
...
select
    ...
    sum(case when seller_favorites_added_start_date > 0 then 1 end) as sellers_favorite_added_start_date,
    sum(case when seller_favorites_removed > 0 then 1 end) as sellers_favorite_removed,
    sum(case when seller_favorites_removed_start_date > 0 then 1 end) as sellers_favorite_removed_start_date,
    ...
    sum(case when seller_contacts_start_date > 0 then 1 end) as sellers_with_contacts_start_date,
