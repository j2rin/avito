select
	...
	case when CallerIsBuyer then 'incoming' when not CallerIsBuyer then 'outgoing' end CallType,
    case when CallEvents like '%1r%' then True else False end as is_receiver_init,
    case when CallEvents like '%2r%' then True else False end as is_receiver_ringing,
    AppCallScenario,
    Reciever_MicAccess,
