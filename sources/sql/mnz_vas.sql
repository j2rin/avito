...
select
	...
	page_from,
		...
		when mv.event_date >= us.converting_date then 'Pro'
		else 'Private' end as
	user_segment,
...
left join /*+jtype(h),distrib(l,a)*/ DMA.user_segments us on mv.user_id = us.user_id and lc.logical_category_id = us.logical_category_id
left join /*+jtype(h),distrib(l,a)*/  usm on mv.user_id = usm.user_id
...
left join /*+jtype(h),distrib(l,a)*/ am_client_day acd on mv.user_id = acd.user_id and mv.event_date between acd.active_from_date and acd.active_to_date
