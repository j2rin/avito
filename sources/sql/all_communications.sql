...
from dma.all_contacts a
    ...
    left join /*+distrib(a,l)*/ DMA.user_segment_market as usm
        ...
        and a.event_date::date = usm.event_date
    ...
    left join (
                select cookie_id, logical_category_id, first_target_contact_event_date
                from dma.target_buyer_birthday
                where cookie_id in (
                    select distinct buyer_cookie_id
                    from dma.all_contacts
                    where cast(event_date as date) between :first_date and :last_date
                        and buyer_cookie_id is not null
                        and buyer_cookie_id <> 0
                        -- and event_month between date_trunc('month', :first_date) and date_trunc('month', :last_date) -- @trino
                )
                    -- and first_target_contact_event_year is not null -- @trino
            ) tbb
        on   a.buyer_cookie_id = tbb.cookie_id
        and  lc.logical_category_id = tbb.logical_category_id
