...
def convert_metrics(old_metrics: List[MetricOld]):
    ...
    duplicated_metrics = old_metrics[0].duplicated_metric_index
    ...
    print('uniq')
    _ = [m.make_ratio() for m in old_metrics if m.type == 'ratio']
    print('ratio')
    for om in [m for m in duplicated_metrics if m.name in {om.name for om in old_metrics}]:
        print(om)
    return list(all_metrics) + [m for m in duplicated_metrics if m.name in {om.name for om in old_metrics}]
