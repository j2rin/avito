    ...
    from (
        ...
        from (
            select  o.participant_id,
                    ...
                    o.ab_period_id,
                    o.breakdown_id,
                    (sum(case when o.observation_name in ({numenator_str}) then o.observation_value else 0 end) > 0)::int as numenator_value,
                    (sum(case when o.observation_name in ({denominator_str}) then o.observation_value else 0 end) >0)::int as denominator_value
